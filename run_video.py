...
if __name__ == '__main__':
    ...
    parser.add_argument('--load-from', type=str, default='checkpoints/depth_anything_v2_metric_vkitti_vits.pth')
    ...
    for k, filename in enumerate(filenames):
        ...
        while raw_video.isOpened():
            ...
            if not ret:
                print("not ret")
                break
            raw_frame = raw_frame[:frame_height, :, :]
            start_time = time.time()
            depth = depth_anything.infer_image(raw_frame)
            end_time = time.time()
